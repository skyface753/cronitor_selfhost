...
class Jobs:
    ...
    async def load_jobs_from_file_to_db(self):
        ...
        # print("TMP", tmp)
        # test = JobCreateInput(id="test", cron="* * * * *", grace_time=5, is_waiting=False, is_running=False, has_failed=False)
        # await Job.prisma().create(test)
        for job in tmp:
            jobInDB = await Job.prisma().find_first(where={"id": job["id"]})
            if not jobInDB:
                grace_time = int(job["grace_time"])
                newJob = JobCreateInput(id=job["id"], cron=job["cron"], grace_time=grace_time, is_waiting=False, is_running=False, has_failed=False)
                # print("New Job", newJob)
                await Job.prisma().create(newJob)
            # Check if grace time and cron changed
            else:
        ...
        print("NEW JOBS", tmp) if DEV else None
    ...
    async def update_job(self, id, had_failed=None, waiting=None, running=None):
        ...
        # Update the job in the internal jobs list
        # for j in self.jobs:
        #     if j["id"] == id:
        #         if had_failed is not None:
    ...
    async def verify_by_id(self, job_id):
        # allJobs = await Job.prisma().find_many()
        # print(allJobs)
        # Check if Job ID is valid
        job = await Job.prisma().find_first(where={"id": job_id}, include={"runsResults": True})
        if not job:
    ...
    async def create_dummy_data(self):
        ...
        await JobRun.prisma().create(JobRunCreateInput(job_id=allJobs[-1].id, started_at="2021-01-01T00:00:00.000Z", finished_at="2021-01-01T00:00:00.000Z", is_success=False, error="failed", command="notARealCommand 'Hallo Welt'", output="Test Message", runtime=4.0))

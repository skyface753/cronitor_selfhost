...
type AllLastForAllJobs struct {
	JobId    string
	ALFAJJob []ALFAJJob
}

type ALFAJJob struct {
	content string
	success bool
}

// func (i *Influx) GetAllLastForAllJobs(ctx context.Context, config *config.Config, justLast bool) (map[string]AllLastForAllJobs, error) {
// 	// query data from influx
// 	query := `from(bucket: "my-bucket")
...
// 	var resultPoints = make(map[string]ALFAJJob)
...
// 			case "success":
// 				if _, ok := result.Record().Value().(bool); !ok {
// 					val.success = false
// 				} else {
